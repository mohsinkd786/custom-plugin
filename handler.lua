...
CustomHandler.PRIORITY = 1003
CustomHandler.VERSION = "0.2.0"
...
local function load_credential(key)
  local creds, err = kong.dao.custom_plugin:find_all {
    key = key
  }
  if not creds then
    return nil, err
...
local function do_authentication(conf)
  ...
  local credential_cache_key = dao.custom_plugin:cache_key(key)
